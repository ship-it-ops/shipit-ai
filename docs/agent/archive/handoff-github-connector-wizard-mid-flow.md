...
**UPDATE 2026-05-24 (this session):** User hit a second wizard UX bug — when adding a second connector for org B with the global App, they pasted org A's installation ID (the only one they had) and the probe authenticated successfully against org A. Connect step had no discovery and no path to install the App in a new org. Shipped the installation picker (see `decisions/github-installation-picker.md`): new `GET /api/connectors/github/installations` endpoint (App-JWT-only Octokit via `createAppJWTOctokit` in connector-github), wizard Connect step replaces the bare textbox with an InstallationPicker showing each org with avatar + "Used by `<id>`" pills + duplicate guard + "Install in another org ↗" link. Manual entry survives behind an Advanced `<details>`. Per-org override mode keeps manual flow. Test posture: 364 total tests green (+3 api-server). NOT YET VERIFIED end-to-end by the user.

**UPDATE 2026-05-24 (continued, same session):**

- User clicked "Install in another org ↗" and landed on `/organizations/ship-it-ops/settings/installations/134528850` — the existing install in their first org. Root cause: install URL was built from GitHub's `html_url` (which for an org-owned App is the App's _settings page within the owner org_; appending `/installations/new` to that path redirects to the existing install). Fixed by always using slug-based public form `https://github.com/apps/<slug>/installations/new`. Same fix in the manifest service's success page. Test mock now uses the org-settings `html_url` form to lock the fix in.
- User then researched and realized the shared-App-across-orgs design requires marking the App **public** in GitHub (the manifest creates `public: false` by default, which only installs in its owner account). User rejected the public-App trade-off. Flipped the wizard's default + ordering: per-org is now the recommended primary path; shared is opt-in with a warning Banner. See `decisions/per-org-github-app-is-default-not-shared.md`.
- User then asked for Step 1 to collect the org name and run the manifest flow scoped to that org (so per-org gets the same one-click create UX shared had). Wired it: manifest `issueState({target, nonce})` now records routing info; `exchangeAndPersist(code, {target, nonce})` routes to either `appService.update()` (global) or `pendingInstance: Map<nonce, creds>` (instance); new `GET /api/connectors/github/manifest/pending-instance/:nonce` returns + clears the entry single-use; wizard's per-org card generates a UUID nonce, opens the manifest with `target=instance`, polls every 2s, and auto-fills `overrideAppId`/`overrideKeyPath` when credentials arrive. See `decisions/github-app-manifest-flow.md` "Update 2026-05-24" section.
- Test posture: now 97 api-server tests (+4 for the instance-target flow), 56 web-ui tests. All 23 turbo tasks green. STILL NOT YET VERIFIED end-to-end by the user — they need to walk through Step 1 → Create App on GitHub (per-org card) → return to wizard and confirm fields auto-fill, then proceed through Connect/Configure/Review.
